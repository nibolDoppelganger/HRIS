...
// Assuming interfaces based on the design
interface Employee {
  id: string;
  name: string;
  email: string;
...
interface AlertCardData {
  id: string;
  type: "contracts" | "birthdays" | "probation";
  title: string;
  subtitle: string;
  count: number;
  icon: string;
...
export const DashboardView: React.FC = () => {
  ...
  useEffect(() => {
    // Initial fetch from our mock api
    const fetchData = async () => {
      try {
        const res = await api.getEmployeesList();
        if (res?.success) {
          // Map to match component structure
          const mappedEmployees = res.data.map((e: any) => ({
            id: e.id,
            name: e.nama,
            email: e.email || '',
            position: e.posisi,
            division: e.departement,
            unit: e.unit,
            status: e.status_kepegawaian.toUpperCase(),
            joinedDate: e.tanggal_bergabung,
            gender: 'Laki-laki', // Default mock
            level: 'Staf', // Default mock
            tenure: '1 tahun', // Default mock
          }));
        ...
        setAlerts([
          ...
          },
          {
            ...
            subtitle: "Berakhir dlm 14 Hari",
            count: 1,
            icon: "how_to_reg",
            actionText: "Evaluasi Staf",
          },
        ]);
        ...
        setActivities([
          {
            ...
            description: "Dian memperbarui alamat domisili",
          ...
          },
          {
            id: "act-2",
            title: "Karyawan Baru",
            description: "Rudi Haryanto bergabung di divisi Program",
          ...
          },
          {
            id: "act-3",
            title: "Approval Cuti",
            description: "Cuti tahunan Siti disetujui manajer",
            time: "Kemarin, 14:00",
            type: "check_circle",
          },
        ]);
  ...
  const tetapCount = employees.filter((e) => e.status === "TETAP").length + baseTetapOffset;
  const kontrakCount = employees.filter((e) => e.status === "KONTRAK").length + baseKontrakOffset;
  const relawanCount = employees.filter((e) => e.status === "RELAWAN").length + baseRelawanOffset;
  ...
  // Divisions mapping
  const programCount = 39;
  const fundraisingCount = 33;
  const operasionalCount = 26;
  const keuanganCount = 13;
  const hrdCount = 7;
  ...
  const onLoadMoreActivities = () => {
    alert("Menampilkan lebih banyak aktivitas...");
  };
    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => alert("Laporan sedang diunduh...")}
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="font-sans text-base font-bold text-[#0b1c30]">
                Tren Kehadiran & Cuti (6 bln)
              </h3>
              <p className="text-[11px] text-[#737686] mt-0.5">Analisis tren ketidakhadiran dan cuti karyawan</p>
            </div>
          ...
          <div className="flex-1 relative min-h-[220px] w-full flex flex-col justify-end">
            ...
            <div className="flex-1 ml-6 relative h-full">
              {hoveredPoint !== null && (
                ...
                >
                  <p className="font-bold">{chartData[hoveredPoint].value} Hari</p>
